...
import { z } from 'zod';
import { calculateAPR, calculateLoan } from '../sdk';
...
loanRoutes.post('/calculate-apr', (req, res): void => {
  const parsed = APRInputSchema.safeParse(req.body);
  ...
  try {
    const result = calculateAPR(parsed.data);
    res.json(result);
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Internal error';
    res.status(500).json({ error: message });
